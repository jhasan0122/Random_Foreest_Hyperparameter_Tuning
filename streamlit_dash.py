...
def train_model(n_estimators, max_depth, max_features, bootstrap):
    rf_classif = RandomForestClassifier(n_estimators=n_estimators, max_depth=max_depth, max_features=max_features,
                                        bootstrap=bootstrap)
    rf_classif.fit(X_train_vec, Y_train)
    return rf_classif

## Dashboard
st.title('Random Forrest :green[Experiment] :tea: :coffee:')
...
with st.form('train_model'):
    ...
    with col1:
        n_estimator = st.slider('No of Estimator',min_value=100,max_value=1000)
    ...
    if submitted:
        rf_classif = train_model(n_estimator, max_depth, max_features, bootstrap)
